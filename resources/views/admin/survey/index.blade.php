    ...
    <div class="grid grid-cols-1 gap-4 sm:gap-6 mb-8 lg:grid-cols-3">
        ...
        <div class="bg-white p-3 sm:p-4 md:p-6 rounded-lg shadow-sm lg:col-span-2">
            <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-4 gap-2">
                ...
                <select class="border rounded-lg px-3 py-1 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-kemenag-light-green">
                ...
                @forelse($surveys->take(5) as $survey)
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ $survey->created_at->format('d M Y - H:i') }}</p>
                        </div>
                    </div>
                @empty
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($surveys as $survey)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-3 sm:px-4 py-3 hidden lg:table-cell">{{ $survey->created_at->format('d M Y H:i') }}</td>
                            <td class="px-3 sm:px-4 py-3 whitespace-nowrap flex items-center gap-2">
                                ...
                                <form action="{{ route('admin.survey.destroy', $survey->id) }}" method="POST" class="inline">
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="inline-flex items-center px-3 py-1.5 text-xs sm:text-sm bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors duration-150"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-3 sm:px-4 py-4 text-center text-gray-500 text-xs sm:text-sm">Belum ada data survey.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="p-3 sm:p-4 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-xs sm:text-sm text-gray-600">
                    Menampilkan <span class="font-medium">{{ $surveys->firstItem() }}</span> -
                    <span class="font-medium">{{ $surveys->lastItem() }}</span> dari
                    <span class="font-medium">{{ $surveys->total() }}</span> survey
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    <div class="hidden sm:flex space-x-2">
                        @foreach ($surveys->getUrlRange(max(1, $surveys->currentPage() - 2), min($surveys->lastPage(), $surveys->currentPage() + 2)) as $page => $url)
                            @if ($page == $surveys->currentPage())
                                <span class="px-3 py-1.5 rounded-md bg-kemenag-green text-white font-medium">{{ $page }}</span>
                            @else
                                <a href="{{ $url }}"
                    ...
                    <div class="sm:hidden flex items-center gap-2">
                        <span class="px-3 py-1.5 rounded-md bg-gray-100 text-gray-700">{{ $surveys->currentPage() }}</span>
                        <span class="text-gray-500">/</span>
                        <span class="text-gray-600">{{ $surveys->lastPage() }}</span>
                    </div>
    ...
    const surveyChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    '#ef4444', '#f59e0b', '#6b7280', '#10b981', '#6366f1'
                ],
                borderRadius: 8,
                borderWidth: 1,
                borderColor: '#e5e7eb'
