    ...
    <form action="{{ route('banner.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $banner->title) }}" required
                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-kemenag-green focus:border-transparent transition">
            @error('title')
        ...
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Status Banner</label>
            <div class="flex items-center gap-8">
                <label class="inline-flex items-center">
                    <input type="radio" name="status" value="1" {{ old('status', $banner->status) ? 'checked' : '' }} class="text-kemenag-green focus:ring-kemenag-green">
                    <span class="ml-2 text-green-700 font-medium">Active</span>
                </label>
                <label class="inline-flex items-center">
                    <input type="radio" name="status" value="0" {{ !old('status', $banner->status) ? 'checked' : '' }} class="text-kemenag-green focus:ring-kemenag-green">
                    <span class="ml-2 text-red-700 font-medium">Nonactive</span>
                </label>
            </div>
            @error('status')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        ...
        <div>
            ...
            @if($banner->image)
                <div class="mt-4 flex flex-col items-start gap-2">
                    ...
                    <img src="{{ asset('storage/'.$banner->image) }}" alt="Banner" class="h-40 rounded-lg shadow border border-gray-200">
                </div>
            @endif
            @error('image')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
